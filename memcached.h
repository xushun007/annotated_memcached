...
typedef struct {
    ...
    struct event_base *base;    /* libevent handle this thread uses */ /* 此线程的libevent句柄 */
    struct event notify_event;  /* listen event for notify pipe */ /* 此事件对象与下面的notify_receive_fd描述符关联，主线程通过此事件通知工作线程有新连接的到来 */
    int notify_receive_fd;      /* receiving end of notify pipe */  /* 与main thread通信的管道(pipe)的接收端描述符 */
    int notify_send_fd;         /* sending end of notify pipe */  /* 与main thread通信的管道(pipe)的发送端描述符*/
    struct thread_stats stats;  /* Stats generated by this thread */ /* 线程相关的统计/状态信息 */
    struct conn_queue *new_conn_queue; /* queue of new connections to handle */ /* 连接队列，由主线程添加，等待工作线程处理 */
    cache_t *suffix_cache;      /* suffix cache */ /* 后缀缓存 */
    uint8_t item_lock_type;     /* use fine-grained or global item lock */ /* 锁类型 */
} LIBEVENT_THREAD;
