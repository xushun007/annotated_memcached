...
/**
 * 每个slab class都有一个双向链表 （链接了该slab class所有正在使用的items的指针）
 * heads指向双向链表的头 ： 插入的时候从这边插入
 * tails指向双线链表的尾 ： 删除的时候从这边删除（LRU算法）
 * sizes[id]: LRU[id]的元素个数
*/
 ...
 */
 ...
 /**
  * 计算item占用空间大小， 空间大小 = item结构体的长度 + 键的长度 + 后缀长度 + 值得长度
  *
  */
 ...
 */
/**
 * 如果item所需的空间不超过memcached所支持的最大缓存单元，返回true
 * 
 * 1. 计算item所需的空间大小
 * 2. 计算item属于哪个slabclass id ，id为0，则不满足空间申请需求
 */
...
/*@null@*/
/**
 * 调试中打印slabclass[slabs_clsid]中LRU链表中的元素信息
 *
 */
...
/** 通过hash表(key)来找到所要的item，如果过期了返回null */
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    /* 检查 item 是否超时. 超时了的item将从哈希表和 LRU链表中删除 */
    if (it != NULL) {
...
/* “释放”超时的item(iter->time >= settings.oldest_live) */
